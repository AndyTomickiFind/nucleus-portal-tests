...
test.describe(`NEW TOPLIST subpage - ${config.name} `, {tag: ['@dev']}, () => {
    ...
    test("Check Override modal", async ({NewToplistPage, components}) => {
        await test.step("Check Override modal", async () => {
            ...
            const countriesComboboxItems: string[] = [
                "Angola",
                "Antarctica",
                "Argentina",
                "Aruba",
            ...
            await components.checkCombobox(NewToplistPage.appliesToCountriesCombobox, countriesComboboxItems);

            await NewToplistPage.checkToplistsResults("override", 10);

            await NewToplistPage.cancelOverrideListButton.click({trial:true});
